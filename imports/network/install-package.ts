...
export const PACKAGE_NAME = "@deep-foundation/capacitor-network"
...
export default async function installPackage(deviceLinkId?) {
  ...
  if (!await getIsPackageInstalled({ deep, packageName: PACKAGE_NAME })) {
    ...
    if (deviceLinkId) {
      if (!await getIsLinkExist({ deep, packageName: "@@deep-foundation/capacitor-network", linkName: "Network" })) {

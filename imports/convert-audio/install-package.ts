...
import insertConvertHandler from "./insert-handler";
dotenv.config();
...
export default async function installPackage() {
  ...
  const { data: [{ id: packageLinkId }] } = await deep.insert({
    ...
    string: { data: { value: CONVERT_AUDIO_PACKAGE_NAME } },
    in: {
      data: [{
  ...
  await insertConvertHandler(deep);
  
  console.log("convert-audio package installed");
}